...
const Widget: Component<{ label: string; icon: string; children: any }> = (p) => (
  <div class="relative bg-stone-900 border border-stone-700">
    ...
    <div class="flex items-center gap-2 px-4 py-2.5 bg-stone-950/50 border-b border-stone-700/60">
      ...
      <span class=" text-[11px] uppercase tracking-[0.2em] text-amber-600">{p.label}</span>
    </div>
...
const SidebarContent: Component<{
  ...
  sortBy: string; onSort: (v: "created_at" | "voti_totali") => void;
  ...
  <div class="space-y-3">
    <Widget label="Cerca server" icon="">
      <input
        ...
        class="w-full px-3 py-2.5 bg-stone-950 border border-stone-700 focus:border-amber-700 text-stone-300  text-sm outline-none transition-colors placeholder:text-stone-600"
      />
    </Widget>

    <Widget label="Modalità di gioco" icon="">
      <div class="flex flex-wrap gap-1.5">
        <For each={TAGS}>{tag => (
          <button
            onClick={() => p.onToggle(tag)}
            class={`px-2.5 py-1.5 text-xs  border transition-all
              ${p.filters.has(tag)
      ...
      <Show when={p.activeCount > 0}>
        <button
          onClick={p.onClear}
          class="mt-3 w-full py-1.5 border border-red-900/50 text-red-700 hover:text-red-500 hover:bg-red-950/20  text-xs uppercase tracking-wider transition-all"
    ...
    <Widget label="Ordina per" icon="">
      <div class="flex flex-col gap-1">
        {(["created_at", "voti_totali"] as const).map(opt => (
          <button
            onClick={() => {
              console.log("Sort messo su: " + opt);
              p.onSort(opt)
            }}
            class={`py-2.5 px-3 text-sm  border text-left flex items-center gap-2.5 transition-all
              ${p.sortBy === opt
          ...
          >
            <span>{opt === "voti_totali" ? "⚔" : opt === "created_at" ? "📜" : "👥"}</span>
            {opt === "voti_totali" ? "Più votati" : opt === "created_at" ? "Più recenti" : "Più giocatori"}
    ...
    <Show when={p.isAuth}>
      <A href="/servers/add"
        class="flex items-center justify-center gap-2 w-full py-3 border border-amber-800/60 bg-gradient-to-r from-amber-950/40 to-stone-900 text-amber-500 hover:text-amber-300 hover:border-amber-600  text-xs uppercase tracking-[0.18em] transition-all"
...
const ServerListPage: Component = () => {
  ...
  const [sortBy, setSortBy] = createSignal<"created_at" | "voti_totali">("voti_totali");
  ...
  const [initialData] = createResource(async () => {
    const result = await ServerService.getServerParams({ page: 1, limit: ITEMS, sort: sortBy + ":desc" });
  ...
  const filtered = createMemo(() => {
    ...
    if (sortBy() === "voti_totali") r = [...r].sort((a, b) => (b.voti_totali ?? 0) - (a.voti_totali ?? 0));
    else if (sortBy() === "created_at") r = [...r].sort((a, b) => new Date(b.created_at ?? 0).getTime() - new Date(a.created_at ?? 0).getTime());
    return r;
  });
    ...
    <div class="relative min-h-screen bg-stone-950 overflow-x-hidden">
      ...
      <div class="relative z-10 border-b-2 border-stone-800 bg-gradient-to-b from-stone-900/70 to-transparent">
        ...
        <div class="h-7 flex items-center justify-center gap-5 border-b border-stone-800/60 bg-stone-950/60 overflow-hidden">
          {RUNES.slice(0, 14).map(r => (
            <span class="text-amber-900/50  text-xs select-none">{r}</span>
          ))}
        </div>

        <div class="max-w-[1600px] mx-auto px-4 sm:px-6 py-6 sm:py-8 flex flex-wrap items-end justify-between gap-4 text-align justify-center">

          {/* header */}
          <div class="text-center mb-10">
            <div class="flex items-center justify-center gap-3 mb-4">
              <div class="h-px w-10 bg-amber-800/60" />
              <span class="text-amber-700 text-xs  uppercase tracking-[0.3em]">Cronache del Regno</span>
              <div class="h-px w-10 bg-amber-800/60" />
            </div>
            <h1 class=" font-black text-5xl text-amber-500 uppercase tracking-widest drop-shadow-[0_2px_12px_rgba(180,100,10,0.4)] mb-2">
              Lista \ Regni Pubblici di hytale
            </h1>
            <p class="text-stone-600 text-xs  uppercase tracking-[0.3em]">
              Pubblicati dalla community
            </p>
          </div>

          {/* mobile filter toggle */}
          <button
            class="lg:hidden flex items-center gap-2 px-5 py-2.5 border border-stone-700 hover:border-amber-800/60 bg-stone-900 text-stone-400 hover:text-amber-400  text-xs uppercase tracking-widest transition-all"
        ...
        <div class="flex gap-0">
          ...
          <aside class="hidden lg:flex flex-col w-72 xl:w-80 flex-shrink-0">
            <div class="sticky top-20 space-y-4">
              <div class="flex items-center gap-2">
                ...
                <span class="text-amber-900/50  text-xs select-none">✦ ᚠ ᚢ ᚦ ✦</span>
                <div class="h-px flex-1 bg-gradient-to-l from-transparent to-amber-900/40" />
              </div>
              <SidebarContent {...sidebarProps()} />
              <div class="pt-1 text-center select-none">
                <span class="text-stone-800  text-2xl tracking-[0.4em]">ᛃ ᛇ ᛈ ᛉ</span>
              </div>
            </div>
          </aside>
          ...
          <main class="flex-1 min-w-0">
            <div class="flex items-center gap-4 mb-6">
              <span class=" text-[11px] uppercase tracking-[0.22em] text-amber-800/70 whitespace-nowrap">
                Lista dei server hytale
              </span>
              <div class="flex-1 h-px bg-gradient-to-r from-amber-900/35 to-transparent" />
              <span class="text-amber-900/35  text-xs select-none">ᛞ ᛟ</span>
            </div>

            <Show when={!initialData.loading} fallback={
              <div class="flex flex-col items-center justify-center py-40 gap-5 text-amber-800 ">
              ...
              <Show when={!initialData.error} fallback={
                <div class="text-center py-32 border border-stone-800 bg-stone-900/30">
                  ...
                  <p class="text-red-700  text-lg">Errore di sistema</p>
                  <p class="text-stone-600  text-sm mt-1">Riprova più tardi</p>
                </div>
              }>
                <Show when={filtered().length > 0} fallback={
                  <div class="relative text-center py-32 border border-stone-800 bg-stone-900/20 overflow-hidden">
                    <span class="absolute top-4 left-4 text-stone-800/30  text-5xl select-none">ᛒ</span>
                    <span class="absolute bottom-4 right-4 text-stone-800/30  text-5xl select-none">ᛖ</span>
                    <p class="text-5xl mb-4">🏰</p>
                    <p class="text-stone-300  text-xl mb-2">Nessun server trovato</p>
                    <p class="text-stone-600  text-sm mb-6">Modifica i filtri di ricerca</p>
                    <button onClick={clearFilters}
                      class="px-8 py-2.5 border border-amber-900/60 text-amber-700 hover:text-amber-400 hover:border-amber-700  text-sm uppercase tracking-widest transition-all">
                      Reset ricerca
                    </button>
                  </div>
                }>
                  ...
                  <Show when={loadingMore()}>
                    <div class="flex flex-col items-center py-10 gap-3">
                      ...
                      <span class="text-xs  text-stone-600 uppercase tracking-wider">Carico altri...</span>
                    </div>
                  </Show>
                  ...
                  <Show when={!hasMore() && allServers().length > 0}>
                    <div class="mt-10 pt-6 border-t border-stone-800/60 flex items-center gap-5">
                      ...
                      <div class="text-center">
                        <p class="text-stone-600  text-xs uppercase tracking-[0.2em]">Fine del registro</p>
                        <p class="text-amber-900/40  text-base mt-1 tracking-widest select-none">✦ ᚠ ᛟ ✦</p>
                      </div>
                      <div class="flex-1 h-px bg-gradient-to-r from-transparent via-amber-900/30 to-transparent" />
                    </div>
                  </Show>
      ...
      <Show when={scrollY() > 500}>
        <button
          ...
          class="fixed bottom-8 right-6 z-50 w-11 h-11 border border-amber-800/60 bg-stone-900/95 text-amber-500 hover:text-amber-300 hover:border-amber-600 transition-all flex items-center justify-center  text-lg shadow-2xl shadow-black/60"
        >↑</button>
      </Show>
